...
public class ShopSetOwnerExecutor extends AbstractShopTransactionCmdExecutor {

    public ShopSetOwnerExecutor(PlayerShopsRPG plugin) {
        super(plugin);
    }
    ...
    public CommandResult execute(CommandSource src, CommandContext args) throws CommandException {
        return super.executeTransaction(src, args, (player, arg, shop) -> {
            Optional<User> userArgOptional = arg.getOne(CommandArgs.USER_ARGUMENT);
            if (userArgOptional.isPresent()) {
                return shop.setOwner(player, userArgOptional.get().getUniqueId());
            }
            return ShopTransactionResult.EMPTY;
        }, "You cannot set shop owners from the console!");
    }
}
